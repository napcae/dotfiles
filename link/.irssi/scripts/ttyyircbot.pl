...
$VERSION = "1";
%IRSSI   = (
    authors     => "Matti \'makinenmo\' Mäkinen",
    contact     => "makinenmo\@gmail.com",
    name        => "ttyyircbot",
    description => "Send notifications about irssi highlights to server",
    license     => "Apache License, version 2.0",
    url         => "https://mattimakinen.net/TTYYIrcBot",
    changed     => "2015-07-18"
);
...
sub send_to_api {
    ...
    if ($pid > 0) {
        ...
        eval {
            my $api_token = Irssi::settings_get_str('ttyyircbot_api_token');
            ...
            if ($type eq 'notification') {
                ...
                $api_url = "https://mattimakinen.net/TTYYIrcBot/Message";
            }
...
sub encrypt {
    my ($text) = @_ ? shift : $_;
    my $password = Irssi::settings_get_str('ttyyircbot_encryption_password');
...
sub are_settings_valid {
    ...
    if (!Irssi::settings_get_str('ttyyircbot_api_token')) {
        Irssi::print("IrssiNotifier: Set API token to send notifications: /set ttyyircbot_api_token [token]");
        return 0;
    }
    ...
    my $encryption_password  = Irssi::settings_get_str('ttyyircbot_encryption_password');
    if (!$encryption_password) {
        Irssi::print("IrssiNotifier: Set encryption password to send notifications (must be same as in the Android device): /set ttyyircbot_encryption_password [password]");
        return 0;
...
Irssi::settings_add_str('ttyyircbot', 'ttyyircbot_encryption_password', 'password');
Irssi::settings_add_str('ttyyircbot', 'ttyyircbot_api_token', '');
